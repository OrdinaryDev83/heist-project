...
{    
    public static class TmxImporter
    {
        ...
        public static void CreateTilesetFromTmx()
        {
            ...
            {
                PlayerPrefs.SetString(k_keyLastOpenFilePanelPath, System.IO.Path.GetDirectoryName(tmxFilePath));
        ...
        {
            string tmxFileName = System.IO.Path.GetFileNameWithoutExtension(tmxFilePath);
            TmxTilemap tilemap = TmxTilemap.LoadFromFile(tmxFilePath);
            if (tilemap.DicTilesetTex2D.Values.Count == 0)
            {
                return null;
            ...
            string tilesetAssetPath = System.IO.Path.Combine(dstPath, tmxFileName + "Tileset.asset");
            ...
            string atlasPath = System.IO.Path.GetDirectoryName(AssetDatabase.GetAssetPath(tilesetAsset)) + "/" + tmxFileName + "Atlas.png";
