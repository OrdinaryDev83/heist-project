Shader "MPUI/Procedural Sprite"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half _FlipVertical;
            ...
            #if CIRCLE
                half circleScene(half4 _additionalData)
                {
                    half2 _texcoord = _additionalData.xy;
                    half width = _additionalData.z;
                    half height = _additionalData.w;
                    half radius = lerp(_CircleRadius, min(width, height) / 2, _CircleFitRadius);
                    half sdf = circle(_texcoord - half2(width / 2, height / 2), radius);
                    return sdf;
                }
            #endif
            
            #if TRIANGLE
                half triangleScene(half4 _additionalData)
                {
                    half2 _texcoord = _additionalData.xy;
                    half width = _additionalData.z;
                    half height = _additionalData.w;
                    
                    half sdf = sdTriangleIsosceles(_texcoord - half2(width / 2, height), half2(width / 2, -height));
                ...
                half pentagonScene(half4 _additionalData)
                {
                    ...
                    half rhombus = sdRhombus(_texcoord - half2(width / 2, _PentagonTipSize * scale), half2(width / 2, _PentagonTipSize) * scale);
            ...
            #if HEXAGON
                half hexagonScene(half4 _additionalData)
                {
                    half2 _texcoord = _additionalData.xy;
                    half width = _additionalData.z;
                    half height = _additionalData.w;
                    ...
                    half rhombus1 = sdRhombus(_texcoord - half2(_HexagonTipSize.x * scale, height / 2), half2(_HexagonTipSize.x, height / 2) * scale);
                    scale = width / _HexagonTipSize.y;
                    half rhombus2 = sdRhombus(_texcoord - half2(width - _HexagonTipSize.y * scale, height / 2), half2(_HexagonTipSize.y, height / 2) * scale);
                    half sdfHexagon = sdfDifference(sdfDifference(baseRect, -rhombus1), -rhombus2);
                    
                    //Left Rounded Corners
                    half halfHeight = height / 2;
                    ...
                    x = (_HexagonCornerRadius.w + circlePivot.x / m - c) / (m + 1 / m); y = m * x + c;
                    ...
                    x = (_HexagonCornerRadius.z + circlePivot.x / m - c) / (m + 1 / m); y = m * x + c;
            ...
            v2f vert(appdata_t v)
            {
                ...
                size = _ConstrainRotation > 0 && abs(_ShapeRotation) % 3.14159 > 0.1? float2(size.y, size.x) : size;
                ...
                shapeUv*= _ConstrainRotation > 0? size : 1;
            ...
            fixed4 frag(v2f IN): SV_Target
            {
                ...
                color = (tex2D(_MainTex, texcoord) + _TextureSampleAdd) * color;
                ...
                #endif
                #if GRADIENT_RADIAL
                    half fac = saturate(length(IN.effectsUv - float2(.5, .5)));
                    ...
                    half sdfData = 0;
                        ...
                        color = lerp(_OutlineColor, color, lerpFac);
                        color.a *= alpha;
                    #endif
